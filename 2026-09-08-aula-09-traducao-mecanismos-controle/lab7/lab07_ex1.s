...
int main (void) { //codigo sempre precisa aparecer debaixo da sessao de texto
  char *pc = S2 [LINHA COMPLEXA] ---> char *pc; pc = S2;
  while (*pc) (while *pc != 0) --> qual é o conteudo apontado por pc? '65'. Esse cara será verdadeiro quando o valor for diferente de zero.
    printf ("%c", *pc++); [LINHA COMPLEXA] --> temp = *pc; printf("%c", temp); pc++;
  printf("\n");
  return 0;
}
Dicionário  
